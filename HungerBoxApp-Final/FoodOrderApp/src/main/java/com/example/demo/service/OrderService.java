...
public class OrderService {
	...
	public OrderResponse createOrder(RedirectAttributes  redirectAttributes,OrderRequestDto orderRequestDto, int userId, String vendor,String cardNumber, int cvv, int month,int year,double amount,long toAccount) throws ParseException {
		...
		if(userEntity.isPresent()) {
			...
			if(!orderRequestDtos.isEmpty()) {
				...
				redirectAttributes.addAttribute("cardNumber", cardNumber);
				redirectAttributes.addAttribute("cvv", cvv);
				redirectAttributes.addAttribute("month", month);
				redirectAttributes.addAttribute("toAccount", toAccount);
				redirectAttributes.addAttribute("year", year);
				redirectAttributes.addAttribute("amount", amount);
				
				String paymentStatus = makePayment( redirectAttributes ,cardNumber,  cvv,  month, year, amount, toAccount);
//				String str = makePayment();
...
//				userOrder.setPaymentId(paymentStatus.RESPONSE_STATUS_ATTRIBUTE);
	...
	private String makePayment() throws ParseException {
		bankClient.test();
		return "hello";

	}

	private String makePayment(RedirectAttributes  redirectAttributes,String cardNumber, int cvv, int month,int year,double amount,long toAccount) throws ParseException {

		return bankClient.payment(redirectAttributes, cardNumber, cvv, month, year, amount, toAccount);
